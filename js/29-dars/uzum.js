const data = [
  ...
  const uzum =document.querySelector('#uzum')
  const selict=document.querySelector('#type')

  selict.addEventListener('change',(e)=>{
  ...
  function foo(){
   for(let i in data){
    ...
    box.appendChild(img)
    img.src=data[i].catalogCard.photos[0].link.high

    
    const p=document.createElement('p')
    box.appendChild(p)
    p.textContent=data[i].catalogCard.__typename

    const span=document.createElement('span')
    box.appendChild(span)
    span.textContent=data[i].catalogCard.title

    const span1=document.createElement('span')
    box.appendChild(span1)
    span1.textContent=data[i].catalogCard.minFullPrice+`so'm`

    
    uzum.appendChild(box)
   }
  }
